...
void write_crash_report(const char *error_message) {
    time_t current_time = time(
            NULL); //la fonction time renvoie nombre de secondes écoulées depuis le 01/01/1970 00:00:00
    struct tm *time = localtime(
            &current_time); //conversion du timestamp précédent en date lisible selon le fuseau horaire local
    char filename[50];
    strftime(filename, sizeof(filename), "%Y-%m-%d_%H-%M-%S_crash_report.txt",
             time); //formatage du nom de fichier en année-mois-jour_heure-min-sec
...
void set_color(Data *data, char *code) {
    if (code[0] == '3') {
        data->cursor.foreground = code[1] - '0';
    } else if (code[0] == '4') {
...
short get_color_pair(Data *data) {
    if (data->colors.color_num == 0) {
        data->colors.colors_pair[0][0] = data->cursor.foreground;
        data->colors.colors_pair[0][1] = data->cursor.background;
        init_pair(8, data->cursor.foreground, data->cursor.background);

        data->colors.color_num++;
        return 1;
    }
    for (int i = 0; i < data->colors.color_num; i++) {
        if (data->colors.colors_pair[i][0] == data->cursor.foreground &&
            data->colors.colors_pair[i][1] == data->cursor.background) {
            return i + 8;
        }
    }
    data->colors.colors_pair[data->colors.color_num][0] = data->cursor.foreground;
    data->colors.colors_pair[data->colors.color_num][1] = data->cursor.background;
    init_pair(data->colors.color_num + 8, data->cursor.foreground, data->cursor.background);
    data->colors.color_num++;
...
void draw_printf(Data *data, const char *format, ...) {
    ...
    for (int i = 0; i < strlen(buffer); i++) {
        if (buffer[i] == '\033') {
            drawText(data->cursor.x, data->cursor.y, str, get_color_pair(data));
            ansi[0] = buffer[i + 2];
            ansi[1] = buffer[i + 3];
            set_color(data, ansi);
